...
function Get-RunnerProcess {
    param([string]$RunnerScriptPath, [string]$ConfigFullPath)
...
if (Test-Path -LiteralPath $runnerPidFile) {
    ...
    if ([int]::TryParse($pidText.Trim(), [ref]$runnerId)) {
        if (Get-Process -Id $runnerId -ErrorAction SilentlyContinue) {
            Write-LogLine -LogPath $logPath -Message "监控已启动，PID=$runnerId，间隔=${intervalSeconds}秒"
            exit 0
        }
    }
...
$existing = Get-RunnerProcess -RunnerScriptPath $runnerScriptPath -ConfigFullPath $configFullPath
if ($existing) {
    Set-Content -LiteralPath $runnerPidFile -Value $existing.ProcessId -Encoding ASCII
    Write-LogLine -LogPath $logPath -Message "监控已启动，PID=$($existing.ProcessId)，间隔=${intervalSeconds}秒"
    exit 0
}
