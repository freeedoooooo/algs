...
- 滑动窗口最大值 -> 监控平台统计最近 5 分钟的峰值 QPS
- 前缀和 -> 日志分析平台快速查询某个时间区间的累计请求数
- Trie -> 搜索框根据用户输入前缀返回候选词
- LRU -> 本地缓存淘汰最近最少使用的数据
- BFS -> 导航系统求最少步数或最短路径
- 并查集 -> 社交账号、设备账号、用户身份合并
...
输入数据的结构是什么？例如数组、日志流、树结构、字符串、图、事件序列。
...
### 12.4 在哪些开源项目或技术中有类似思想

这一节建议不要只列名字，而是补一句“像在哪里”。

推荐写法：

- Redis：`LRU / LFU` 用在缓存淘汰策略
- Elasticsearch：倒排索引、前缀检索、分词匹配
- Kafka：消息分区、顺序消费、批量处理窗口
- Nginx：请求路由、连接调度、限流窗口
- 数据库索引：B+ 树、范围查询、排序访问
- 编译器：词法分析、语法分析、状态机匹配
- 搜索引擎：Trie、倒排索引、字符串匹配
- 操作系统调度：队列、优先级、时间片管理
- 图计算系统：最短路径、连通性分析
...
> 这道题真正解决的是：在 `________` 这个现实约束下，用更高效的方法得到 `________`。
...
> 在推荐系统里，候选召回模块经常会维护多个排序条件下的候选排列。  
> 当当前排序方案不可用或需要继续搜索下一个备选顺序时，可以用这道题背后的“下一个字典序状态”思想，在不枚举所有组合的前提下快速生成下一组候选排序顺序。
