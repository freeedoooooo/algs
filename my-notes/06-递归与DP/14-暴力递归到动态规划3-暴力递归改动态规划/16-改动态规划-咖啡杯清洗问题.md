...
- `dp[1][0] = 5`
  表示只剩最后一只杯子，洗杯机现在空着，最早 5 时刻全部结束
- `dp[1][4] = 6`
  表示只剩最后一只杯子，但洗杯机要到 4 时刻才空出来，这时最早 6 时刻结束
- `dp[0][0] = 5`
  这就是整道题的答案

### 第四步：挑两个格子看它是怎么来的
#### 1. 先看最后一行 `dp[1][4]`
只剩最后一只杯子，当前杯子出现时间是 `3`。
...
#### 2. 再看 `dp[0][2]`
现在处理第 0 只杯子，洗杯机最早 2 时刻可用。

当前杯子机器洗：

```text
wash = max(1, 2) + 2 = 4
p1 = max(4, dp[1][4]) = max(4, 6) = 6
```
