...
因为一旦已经选了 `V-1` 条边，并且整个过程中始终没有成环，那么这批边就已经满足：
...
所以对一张有 `V` 个节点的图来说，只要已经合法选出 `V-1` 条边，就可以立刻停止。
...
因为如果图是连通的，总能找到办法把所有点逐步连成一棵树；  
现在边都耗尽了还不够，说明至少有一部分点根本连不到一起。

所以这时算法也会结束，只不过得到的不是最小生成树，而是：
