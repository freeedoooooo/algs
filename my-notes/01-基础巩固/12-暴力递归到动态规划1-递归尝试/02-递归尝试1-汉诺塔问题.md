...
调用 `hanoi(4, 左, 右, 中)`：**把4层从左柱移到右柱，借助中柱**

```mermaid
graph TD
    A["hanoi(4, 左, 右, 中)"] --> B["步骤1: hanoi(3, 左, 中, 右)"]
    A --> C["步骤2: 📤 move 4 from 左 to 右"]
    A --> D["步骤3: hanoi(3, 中, 右, 左)"]
    
    B --> B1["hanoi(2, 左, 右, 中)"]
    B --> B2["📤 move 3 from 左 to 中"]
    B --> B3["hanoi(2, 右, 中, 左)"]
    
    B1 --> B1a["hanoi(1, 左, 中, 右)"]
    B1 --> B1b["📤 move 2 from 左 to 右"]
    B1 --> B1c["hanoi(1, 中, 右, 左)"]
    
    B1a --> B1a_out["📤 move 1 from 左 to 中"]
    B1c --> B1c_out["📤 move 1 from 中 to 右"]
    
    B3 --> B3a["hanoi(1, 右, 左, 中)"]
    B3 --> B3b["📤 move 2 from 右 to 中"]
    B3 --> B3c["hanoi(1, 左, 中, 右)"]
    
    B3a --> B3a_out["📤 move 1 from 右 to 左"]
    B3c --> B3c_out["📤 move 1 from 左 to 中"]
    
    D --> D1["hanoi(2, 中, 左, 右)"]
    D --> D2["📤 move 3 from 中 to 右"]
    D --> D3["hanoi(2, 左, 右, 中)"]
    
    D1 --> D1a["hanoi(1, 中, 右, 左)"]
    D1 --> D1b["📤 move 2 from 中 to 左"]
    D1 --> D1c["hanoi(1, 右, 左, 中)"]
    
    D1a --> D1a_out["📤 move 1 from 中 to 右"]
    D1c --> D1c_out["📤 move 1 from 右 to 左"]
    
    D3 --> D3a["hanoi(1, 左, 中, 右)"]
    D3 --> D3b["📤 move 2 from 左 to 右"]
    D3 --> D3c["hanoi(1, 中, 右, 左)"]
    
    D3a --> D3a_out["📤 move 1 from 左 to 中"]
    D3c --> D3c_out["📤 move 1 from 中 to 右"]
    
    style C fill:#ff4757
    style B2 fill:#ffa502
    style D2 fill:#ffa502
    style B1b fill:#2ed573
    style B3b fill:#2ed573
    style D1b fill:#2ed573
    style D3b fill:#2ed573
    style B1a_out fill:#1e90ff
    style B1c_out fill:#1e90ff
    style B3a_out fill:#1e90ff
    style B3c_out fill:#1e90ff
    style D1a_out fill:#1e90ff
    style D1c_out fill:#1e90ff
    style D3a_out fill:#1e90ff
    style D3c_out fill:#1e90ff
```
...
2. move 2 from 左 to 右
3. move 1 from 中 to 右
4. move 3 from 左 to 中
5. move 1 from 右 to 左
6. move 2 from 右 to 中
7. move 1 from 左 to 中

第8步（核心）：
8. move 4 from 左 to 右  ← 移动最大盘

后7步（步骤3展开）：
9. move 1 from 中 to 右
10. move 2 from 中 to 左
11. move 1 from 右 to 左
12. move 3 from 中 to 右
13. move 1 from 左 to 中
14. move 2 from 左 to 右
15. move 1 from 中 to 右
```
