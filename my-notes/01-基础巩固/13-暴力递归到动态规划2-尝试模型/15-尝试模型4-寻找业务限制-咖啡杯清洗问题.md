...
### washLine 的上界怎么求
动态规划最大的难点，不是状态定义，而是第二维 `washLine` 不能无限大。
...
这就是递归 base case 直接搬进表里。

### 普通位置怎么填
对于任意 `index` 和 `washLine`：
...
## 递归与动态规划对比
- 当前笔记前半部分讲的是递归建模：重点是先找出 `washLine` 这个业务限制。
- 真正写成 DP 时，状态本身没有变，还是 `index + washLine`，只是把“递归调用”改成了“表格填值”。
- 递归版更适合理解状态设计。
- DP 版更适合真正求解和控制复杂度。
